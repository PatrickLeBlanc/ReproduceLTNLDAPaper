...
# K = 3 #
#########
load("../Results/Data/Markov Chains/Sank_C_8_K_ 3 _data_results.rda")
...
# K = 4 #
#########
load("../Results/Data/Markov Chains/Sank_C_8_K_ 4 _data_results.rda")
...
#permute topics to correct for label switching as K changes
top_perm = c(5,1,7,4)
for(k in 1:K){
  for(d in 1:D){
    props = c(props,post_phi_dk[d,k])
...
Data_abun_df = data.frame(props,samps,tops,model,Subcommunities)

#make new permutation of subcommunities to put most significant ones first
top_perm = c(1,7,6,4,2,5,3)
for(i in 1:nrow(Data_abun_df)){
  Data_abun_df$tops[i] = top_perm[Data_abun_df$tops[i]]
}

save(Data_abun_df,file="../Results/Data/Markov Chains/Data_abun_df.rda")
...
#make dataframe
Data_dist_df = data.frame(input,vals,samp, Distribution,tops)

#reorder for K = 7, same as above
Data_dist_df$tops_f = factor(Data_dist_df$tops, levels=c('1','5','7','2','6','3','4'))
top_perm = c(1,7,6,4,2,5,3)
for(i in 1:nrow(Data_dist_df)){
  Data_dist_df$tops[i] = top_perm[Data_dist_df$tops[i]]
}

save(Data_dist_df,file="../Results/Data/Markov Chains/Data_dist_df.rda")
