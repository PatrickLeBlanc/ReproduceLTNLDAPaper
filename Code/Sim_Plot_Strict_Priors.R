...
load("../../Simulations/Response/Sim_Strict_Priors_C10_TrueK4_K 4 .rda")
post_phi_strict_dk = model$Mean_Post_Phi_d
K = dim(post_phi_dk)[2]
D = dim(post_phi_dk)[1]

post_beta_strict_kv = model$Mean_Post_Beta_k
...
for(k in 1:K){
  for(d in 1:D){
    ...
    props = c(props,post_phi_strict_dk[d,k])
    samps = c(samps,d)
    tops = c(tops,dat_perm[k])
    mod = c(mod,"Strict LTN-LDA")
    Subcommunities = c(Subcommunities,toString(K))
...
#load independent covariance LTN-LDA
load("../../Simulations/Response/Sim_Strict_Priors_C10_TrueK4_K 5 .rda")
post_phi_strict_dk = model$Mean_Post_Phi_d
K = dim(post_phi_dk)[2]
D = dim(post_phi_dk)[1]
...
#load independent covariance LTN-LDA
load("../../Simulations/Response/Sim_Strict_Priors_C10_TrueK4_K 7 .rda")
post_phi_strict_dk = model$Mean_Post_Phi_d
K = dim(post_phi_dk)[2]
D = dim(post_phi_dk)[1]
...
#load independent covariance LTN-LDA
load("../../Simulations/Response/Sim_Strict_Priors_C10_TrueK4_K 10 .rda")
post_phi_strict_dk = model$Mean_Post_Phi_d
K = dim(post_phi_dk)[2]
D = dim(post_phi_dk)[1]
...
#correct label switching
dat_perm = c(7,8,3,9,5,6,1,2,4,10)
